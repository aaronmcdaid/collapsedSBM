...
void cliquePercolation4(const SimpleIntGraph &g_, const string &outputDirectory, unsigned int minimumSize, const vector< pair<double,bool> > &option_thresholds) {
	...
	PP(maxCliqueSize);
	{
		Timer timer("clique-size frequencies");
				...
				if(size_of_overlap > 1) {
					...
					for(int k = 3; k <= size_of_overlap+1; k++) {
						// if(cliqueID_size >= k)
						{
							// assert(k <= cliqueID_size);
							amd::ConnectedComponents &cpmk = cpms.at(k);
							if(cpmk.component.at(clid1) != cpmk.component.at(clid2)) {
								// PP(currentClique);
								// PP(cpmk.component.at(currentClique));
								// if(do_asserts) assert(cliqueID_size >= k);
								// if(do_asserts) assert((int)all_cliques.at(currentClique).size() >= k);
								// if(do_asserts) assert(cliqueID_size <= (int)all_cliques.at(currentClique).size());
								// if(do_asserts) assert(consecutiveLikeThis >= k-1);
								cpmk.joinNodesIntoSameComponent(clid1, clid2);
								// PP(cpmk.component.at(currentClique));
							}
						}
					}
