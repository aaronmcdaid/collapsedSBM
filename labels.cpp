...
typedef pair<int, pair<int,vector<int> > > kz_t; // K, (burnt)iteration, z
...
const pair<int64_t, vector<int> > find_best_labellings(const int K, const vector< vector<int64_t> > &k_by_k) {
	...
	if(best_relabellings_so_far.size()>1)
...
int main() {
	...
	For(kz, kzs) {
		...
		PP4(num_kz_so_far, K, kz->second.first, kz->second.second.at(0));
		...
		if(kz == kzs.begin()) { // just leave the first one as is
			...
			{ // print the relabelled form
				...
				for(int n=0; n<N; n++) {
					...
					cout << ' ' << rel_z_i;
					++ cluster_sizes.at(rel_z_i);
				}
				cout << '\t';
				For(sz, cluster_sizes) {
					cout << ' ' << *sz;
				}
				cout << endl;
			}
		...
		for(int n=0; n<N; n++) {
			relabelled_z.push_back(relabelling.at(kz->second.second.at(n)));
		}
		for(int n=0; n<N; n++) {
